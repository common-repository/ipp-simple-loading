...
;// CONCATENATED MODULE: ./node_modules/react-ready-state-effect/dist/ready-state-effect.esm.js
...
function build_module_apiFetch(options) {
  ...
  return enhancedHandler(options).catch(error => {
    ...
    .fetch(build_module_apiFetch.nonceEndpoint).then(checkStatus).then(data => data.text()).then(text => {
      // @ts-ignore
      build_module_apiFetch.nonceMiddleware.nonce = text;
      return build_module_apiFetch(options);
    });
  });
}

build_module_apiFetch.use = registerMiddleware;
build_module_apiFetch.setFetchHandler = setFetchHandler;
build_module_apiFetch.createNonceMiddleware = nonce;
build_module_apiFetch.createPreloadingMiddleware = preloading;
build_module_apiFetch.createRootURLMiddleware = root_url;
build_module_apiFetch.fetchAllMiddleware = fetch_all_middleware;
build_module_apiFetch.mediaUploadMiddleware = media_upload;
/* harmony default export */ var build_module = (build_module_apiFetch);
//# sourceMappingURL=index.js.map
...
;// CONCATENATED MODULE: ./src/public/loading-screen/index.tsx




var loading_screen_useState = react.useState,
  loading_screen_useEffect = react.useEffect;
var LoadingScreen = function () {
  var _a = loading_screen_useState(false),
    complete = _a[0],
    setComplete = _a[1];
  //@ts-ignore
  var settings = window.ippLoadingSettings;
  loading_screen_useEffect(function () {
    var inlineStyleTag = document.querySelector("#ipp_simple_loading_screen_inline_style");
    if (inlineStyleTag) {
      inlineStyleTag.remove();
    }
    var body = document.body;
    if (body) {
      if (settings.preventScrollFromLoading) {
        body.style.overflow = "hidden";
      }
    }
  }, []);
  useReadyStateEffect(function () {
    setTimeout(function () {
      return setComplete(true);
    }, Number(settings.fixedDelay));
  }, [], "complete");
  return (0,jsx_runtime.jsx)(loading, {
    isFull: true,
    isLoading: !complete,
    settings: settings,
    exitCallBack: function (arg) {
      return setComplete(true);
    }
  });
};
/* harmony default export */ var loading_screen = (LoadingScreen);
;// CONCATENATED MODULE: ./src/public/main.tsx



var observer = new MutationObserver(function () {
  try {
    var loadingScreenDiv = document.querySelector("#ipp_simple_loading_screen");
    if (loadingScreenDiv) {
      client/* createRoot */.s(loadingScreenDiv).render((0,jsx_runtime.jsx)(loading_screen, {}));
      observer.disconnect();
    }
  } catch (e) {
    var inlineStyleTag = document.querySelector("#ipp_simple_loading_screen_inline_style");
